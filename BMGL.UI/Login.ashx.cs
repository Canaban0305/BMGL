    ...
    public class Login : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (validateCode.Equals(vcode, StringComparison.OrdinalIgnoreCase)) //不区分大小写
            {
                ...
                if (model != null)
                {
                    //1024新增：将用户登录信息保存到Cookie中，以实现免登录功能
                    //判断用户是否勾选了免登录复选框
                    ...
                    //新增:将登录用户名保存到Session中，以便在首页中使用
                    context.Session["LoginUser"] = userName;

                    context.Response.Write("<script>alert(Session[\"LoginUser\"]);</script>");
                    //跳转
                    context.Response.Write("<script>alert('登录成功!');window.location.href='Hpage.aspx'</script>");
                } 
                else
                {
